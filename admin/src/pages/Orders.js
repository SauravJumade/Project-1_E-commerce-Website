...
function Orders() {
    ...
    const fetch = async (axiosCancelToken) => {
        try {
            const queryStr = queries ? new URLSearchParams(queries) : "";
           // const {data} = await req.get(`/api/orders/?page=${page}&${new URLSearchParams(queries)}`, {cancelToken: axiosCancelToken.token})
           const {data} = await req.get(`/api/orders/?page=${page}&${queryStr}`, { cancelToken: axiosCancelToken.token });
            setOrders(p => [...p, ...data])
        } catch (error) {
            if(error.response?.status === 404) 
                return  dispatch(setError(error.response.data.message))
            if(axios.isCancel(error)) 
                return setOrders([]) //req canceled by user
